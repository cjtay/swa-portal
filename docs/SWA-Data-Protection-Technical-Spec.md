...
| 2026-07-15 (eve) | Wrap-up sweep after the removal: confirmed no `backup*.sql`/`*.age` files or age keys remain, `age` binaries uninstalled, `scripts/backup-prod.sh` gone; deleted the stale `*.config/age/*` deny rule from opencode; added `.zcode/` to the repo `.gitignore`; re-ran all §5 verification procedures (pass); specs finalised and committed. |
