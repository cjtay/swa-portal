...
> **Status**: **Batches A and B built** (2026-09-05) — Batch A: migration 012 + schema backport, self-approval guard, decision offices, S$5,000 two-stage force, invoice number + duplicate warning, R4 (GIRO), R5 (voucher print), the R7 `is_tax_invoice` column, R8 matrix printed in the spec + guide. Batch B: migration 013 + schema backport, S$1,000 declarations + quotations-or-waiver, quotation dates + 12-month warning, S$10,000 board guard, R1 field-level audit, R6 remembered payment method, R7 checkbox UI. Tests + docs landed with each batch. **Batch C remains — build next** (§17.2). Change-request document: `docs/plans/approvals-additional-requirements-2026-08-29.md`.
