...
Session flags `is_purchase_approver` / `is_finance_approver` / `is_approvals_viewer` reach the browser via `/api/session` and drive the Approvals nav item and board actions. Ordinary committee members have no access — financial data.
...
| `GET /api/approvals/export?status=<status>` | GET | Admin tier (IT admins hold the admin role) | **R3**: board-list CSV — one row per item (voucher no, title, payee, category, amount, status, dates, decision makers), filtered by the given status tab, newest first, ≤5,000 rows, injection-guarded. Reached from the board's Export CSV button, which exports the open tab. The R2 auditor cannot export (403, proven by test) |
...
|---------|-------------|
| Page itself + Approvals nav item | `is_admin` OR either approver flag OR `is_approvals_viewer` (R2 — auditor sees the same read-only board; else redirect `/`) |
| Status tabs with count badges | Always (For approval, Approved, In finance check, Finance approved, Rejected, Paid, All). Default tab: **All**. Auditors can filter freely (R2) |
| "New request" button | `is_admin` only. The board table leads the page; the form opens on demand and collapses on Cancel or successful submit (owner decision 25-08-2026). Never shown to auditors |
