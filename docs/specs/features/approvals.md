...
- **Documents**: PDF/JPG/PNG/WebP/HEIC/HEIF, 10 MB each, 10 files per item; HTML and SVG always rejected. Files accumulate across multiple picker visits; viewed inline (iframe for PDFs). A create/edit request is one POST, so `wrangler.jsonc` caps the whole body at 110 MB (raised from 20 MB on 2026-08-28 — the old cap 413-rejected legitimate multi-file uploads before the per-file limits could run; effective ceiling is also the plan's ~100 MB free-plan body limit).
